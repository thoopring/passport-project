...
export const REFUND_CONTENT: Record<Locale, RefundContent> = {
  ...
  ko: {
    ...
    summaryHeading: "요약",
    summaryList: [
      "결제 후 10분 이내 기술적 사유로 플랜 생성에 실패한 경우 자동 환불.",
      "플랜에 명백히 환각된 장소(해당 위치에 실제로 존재하지 않는 식당·호텔·관광지)가 포함된 경우, 14일 이내 신청 시 환불.",
      "취향 차이(추천이 마음에 들지 않음), 결제 후 여행 일정 변경, PDF 다운로드 후 14일 경과 시 환불 불가.",
    ],
    sections: [
      {
        heading: "1. 기술적 실패 환불",
        bodies: [
          "결제 성공 후 10분 이내 시스템이 플랜을 전달하지 못한 경우 — 예: Claude 타임아웃, 이메일 발송 실패, 시스템에서 플랜이 failed로 표시 — LemonSqueezy를 통해 원래 결제 수단으로 자동 환불을 진행합니다. 별도 연락 필요 없습니다. 로그에서 실패를 확인하고 처리합니다.",
          "환불은 보통 카드사에 따라 5~10영업일 안에 명세서에 반영됩니다. 10영업일 후에도 환불이 도착하지 않으면 LemonSqueezy 주문 번호와 함께 gliddy@checkvisamap.com 으로 메일 주세요.",
        ],
      },
      {
        heading: "2. 환각 환불",
        bodies: [
          "AI 생성 플랜은 드물게 해당 위치에 존재하지 않는 장소나 영구 폐업한 장소를 포함할 수 있습니다. 엄격한 프롬프트 규칙과 큐레이션된 장소 데이터베이스로 이를 막기 위해 노력하지만, 완벽한 생성은 없습니다.",
          "플랜을 열어보고 환각된 장소를 발견하시면, 결제 후 14일 이내에 플랜 배송 메일에 회신해 주세요. 코스 이름과 증거(해당 주소에 그 장소가 존재하지 않음을 보여주는 검색 결과 등)를 포함해 주시면 됩니다. 플랜 전액 환불해 드립니다.",
        ],
      },
      {
        heading: "3. 환불 불가 사유",
      ...
      {
        heading: "5. 차지백",
        body: "저희에게 먼저 연락하지 않고 카드사에 차지백을 신청하시면 배송 증거와 함께 이의를 제기합니다. 그냥 메일 주시는 것이 훨씬 좋습니다 — 사유가 맞으면 빠르게 환불해 드리고 모두에게 스트레스가 적은 절차입니다.",
      },
    ],
    contactHeading: "6. 문의",
